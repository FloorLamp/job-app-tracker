...
(function() {
  $(document).on('ready page:load', function() {
    var FAVICON_URL, company_favicon_img, company_icon_url_input, company_name_input, company_name_input_timeout, company_url_input, company_url_input_timeout;
    ...
    company_name_input.on('input', function() {
      clearTimeout(company_name_input_timeout);
      return company_name_input_timeout = setTimeout(function() {
        return $.getJSON('/company_url.json?name=' + company_name_input.val(), function(data) {
          company_url_input.val(data.url);
          company_icon_url_input.val(FAVICON_URL + data.url);
          return company_favicon_img.attr('src', FAVICON_URL + data.url);
        });
      }, 500);
    });
    return company_url_input.on('input', function() {
      clearTimeout(company_url_input_timeout);
      return company_url_input_timeout = setTimeout(function() {
        company_icon_url_input.val(FAVICON_URL + company_url_input.val());
        return company_favicon_img.attr('src', FAVICON_URL + company_url_input.val());
      }, 500);
    });
  });
